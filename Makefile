...
# Lista de todos os diretórios encontrados recursivamente, exceto "TaskTrace"
SRC_DIRS_SUP := $(shell find . -type d ! -name TaskTrace)

# Lista de todos os arquivos .c, exceto os encontrados em "TaskTrace"
SRCS_SUP := $(foreach dir,$(SRC_DIRS_SUP),$(filter-out $(dir)/TaskTrace/%.c,$(wildcard $(dir)/*.c)))
...
## Definições para compilação da lib do usuário (TaskTrace).
...
# Nome do alvo final
TARGET_USR = TaskTrace/TaskTrace
